...
namespace OM.MFPTrackerV1.Data.Services
{
	...
	public class FolioHolderRepo : IFolioHolderRepo
	{
			...
			CancellationToken ct = default)
		{
			...
			{
				var pattern = $"%{signatureContains}%";
				q = q.Where(x => x.Signature != null && EF.Functions.Like(EF.Functions.Collate(x.Signature, "NOCASE"), pattern));
			}
			...
			{
				("FirstName", true) => q.OrderByDescending(x => EF.Functions.Collate(x.FirstName, "NOCASE")).ThenBy(x => x.FolioHolderId),
				("FirstName", false) => q.OrderBy(x => EF.Functions.Collate(x.FirstName, "NOCASE")).ThenBy(x => x.FolioHolderId),

				("LastName", true) => q.OrderByDescending(x => EF.Functions.Collate(x.LastName, "NOCASE")).ThenBy(x => x.FolioHolderId),
				_ => q.OrderBy(x => EF.Functions.Collate(x.LastName, "NOCASE")).ThenBy(x => x.FolioHolderId),
			};
